...
function cssVar(name: string) {
  // SSR kontrolü ekledik (Next.js build sırasında patlamasın diye)
...
export default function AnalyticsChart() {
  ...
  useEffect(() => {
    ...
    const draw = () => {
      ...
      // ✅ ÇÖZÜM: Parametreyi 'readonly number[]' yaparak TS hatasını giderdik
      const drawLine = (series: readonly number[], stroke: string, fill: string) => {
  ...
  return (
    <div className="card" style={{ marginTop: 16 }}>
      <div className="card-h">
        <div>
          <h2>Analytics</h2>
          <span style={{ color: "var(--muted)", fontSize: "14px" }}>Revenue & orders trend</span>
        </div>
        ...
        <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
          {(["7d", "30d", "90d"] as const).map((r) => (
            <button
              key={r}
              className="btn"
              style={{ padding: "8px 10px", ...(range === r ? activeBtnStyle : {}) }}
              onClick={() => setRange(r)}
              type="button"
            >
              {r}
      ...
      <div style={{ padding: 14 }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", gap: 10, marginBottom: 10 }}>
          <div style={{ display: "flex", gap: 14, flexWrap: "wrap" }}>
            <span className="pill" style={{ padding: "7px 10px", display: "flex", alignItems: "center", gap: 6 }}>
